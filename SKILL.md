...
| 前置条件 | 才能进入 |
|----------|----------|
| Step 1 输出代码结构 + 调用链 + 确认范围 | → Step 2 |
| Step 2 输出影响分析（上游/下游/边界） | → Step 3 |
| Step 3 输出改动项拆分（带位置） | → Step 4 |
| Step 4 输出结构化清单 + 排序 | → Step 5 |
| Step 5 用户确认清单 | → Step 6 |
| Step 6-7 逐项执行 + 输出文档 | → 完成 |
...
| 想法 | 真实情况 |
|------|----------|
| "时间紧，直接改吧" | 必须先走完 Step 1-5，否则遗漏风险高 |
| "改动很简单" | 简单也要分析影响，防止牵连其他模块 |
| "用户催了" | 先确认清单再执行，避免做错重来 |
| "我先改代码再补文档" | 文档要先输出，防止实现偏离需求 |
| "影响分析没必要" | 影响分析是防止遗漏调用方的核心步骤 |
| "用户说清楚了，不用确认" | 用户理解 ≠ 代码理解，必须输出代码结构 |
| 跳过任何 Step | 停止，回到上一步，输出要求内容 |
...
2. **Read 理解代码** — 整个文件或相关函数，理解：
   ...
   - 依赖的下游服务/模块
   - 边界条件、异常处理
3. **输出必须内容** — 以下内容**必须全部输出**才能进入 Step 2：
   - ✅ 找到的文件路径
   - ✅ 代码结构（入口函数、参数、返回值）
   - ✅ 调用链（树形结构）
   - ✅ 依赖关系
...
**必须输出内容** — 以下内容**必须全部输出**才能进入 Step 3：
- ✅ 上游调用方列表（文件 + 函数）
- ✅ 下游依赖列表（文件 + 函数）
- ✅ 边界条件分析
- ✅ 潜在风险标注
...
**拆项判断标准（严格执行）：**
- 用户提到的**每个改动点** → 都要拆成具体改动项
- 即使现有代码已有类似实现 → 也要拆项并标注"已有XX，是否需要增强？"
- **不要自行判断"不需要改"** → 让用户确认
- 拆项时对应用户原始表述，不要遗漏任何一个点

**必须输出内容** — 以下内容**必须全部输出**才能进入 Step 4：
- ✅ 改动项列表（带编号）
- ✅ 每项标注：类型、位置（文件:函数:行号）、描述
- ✅ 与用户原始需求的对应关系（如：#1 对应用户说的"记录回调参数"）
- ✅ 依赖关系标注
...
**必须输出内容** — 以下内容**必须全部输出**才能进入 Step 5：
- ✅ 按模块/业务线分组的表格
- ✅ 每组标注依赖关系
- ✅ 执行顺序说明（并行/串行）
- ✅ 改动项总数统计
...
向用户展示清单 + 影响分析，**等待用户确认后才能进入 Step 6**。
...
**前置条件**：Step 6 执行完成（或有用户明确要求先输出文档）。
